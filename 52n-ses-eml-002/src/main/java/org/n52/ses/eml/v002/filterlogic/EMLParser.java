...
package org.n52.ses.eml.v002.filterlogic;
...
import net.opengis.eml.x002.AbstractGuardedViewPatternType;
import net.opengis.eml.x002.AbstractPatternType;
import net.opengis.eml.x002.AbstractViewPatternType;
import net.opengis.eml.x002.EventAttributeType;
import net.opengis.eml.x002.GenericViewType;
import net.opengis.eml.x002.GenericViewType.InsertCriteria.InsertCriterion;
import net.opengis.eml.x002.GenericViewType.ParameterDefinitions.ParameterDefinition;
import net.opengis.eml.x002.GenericViewType.ReleaseCriteria.ReleaseCriterion;
...
import net.opengis.eml.x002.UserDefinedOperatorType;
import net.opengis.eml.x002.UserParameterType;
import net.opengis.eml.x002.ViewType;
import net.opengis.eml.x002.ComplexPatternDocument.ComplexPattern;
import net.opengis.eml.x002.EMLDocument.EML;
import net.opengis.eml.x002.SelectFunctionType.NotifyOnSelect;
import net.opengis.eml.x002.SelectFunctionType.SelectAvg;
import net.opengis.eml.x002.SelectFunctionType.SelectEvent;
import net.opengis.eml.x002.SelectFunctionType.SelectMax;
import net.opengis.eml.x002.SelectFunctionType.SelectMin;
import net.opengis.eml.x002.SelectFunctionType.SelectProperty;
import net.opengis.eml.x002.SelectFunctionType.SelectSum;
import net.opengis.eml.x002.SelectFunctionType.UserDefinedSelectFunction;
import net.opengis.eml.x002.TimerPatternType.TimerAt;
import net.opengis.eml.x002.ViewType.LengthView;
import net.opengis.eml.x002.ViewType.TimeLengthView;
import net.opengis.eml.x002.ViewType.TimeView;
import net.opengis.eml.x002.ViewType.UserDefinedView;
import net.opengis.fes.x20.FilterType;

import org.apache.xmlbeans.GDuration;
import org.n52.ses.api.eml.ILogicController;
import org.n52.ses.eml.v002.Constants;
import org.n52.ses.eml.v002.pattern.AGuardedViewPattern;
import org.n52.ses.eml.v002.pattern.APattern;
import org.n52.ses.eml.v002.pattern.AViewPattern;
import org.n52.ses.eml.v002.pattern.DataView;
import org.n52.ses.eml.v002.pattern.PatternComplex;
import org.n52.ses.eml.v002.pattern.PatternGuard;
import org.n52.ses.eml.v002.pattern.PatternOperator;
import org.n52.ses.eml.v002.pattern.PatternOutputReference;
import org.n52.ses.eml.v002.pattern.PatternRepetitive;
import org.n52.ses.eml.v002.pattern.PatternSimple;
import org.n52.ses.eml.v002.pattern.PatternTimer;
...
public class EMLParser {
	...
	public void parseEML(EML eml) {
		EMLParser.logger.debug("parsing EML document");
		/*
		...
		for (SimplePatternType pattern : sPatterns) {
			...
			try {
				...
				EMLParser.logger.warn(e.getMessage(), e);
			}
		}
	 ...
	 */
	private void parseSimplePattern(SimplePatternType pattern) throws UnsupportedEncodingException {
		...
		//set input name
		if (pattern.getInput().isSetExternalInput()) {
			simplePattern.setInputName(pattern.getInput().getExternalInput()
					.replaceAll(":", "__").replaceAll("\\.", "_"));
		} else {
			throw new UnsupportedEncodingException("currently only ExternalInput is supported.");
		}
		
		
		//set property restrictions
		if (pattern.getPropertyRestrictions().sizeOfPropertyRestrictionArray() > 0) {
	...
	private void parseViewPattern(AViewPattern representation, AbstractViewPatternType bean) {
		//set view
		if (bean.isSetView()) {
			DataView view = this.parseView(bean.getView());
			
			/**
			 * HACK for dynamic select function.... used in the generic view
			 * to determine what kind of dynamic filter is used.
			 * really bad hack due to bad EML modeling I guess.
			 */
			view.addParameter("SelectFunctions", representation.getSelectFunctions());
...
//				}
		...
		//else user defined function
			...
			EMLParser.logger.debug("parsing user defined sel function");
	...
	private DataView parseView(ViewType view) {
		...
		//generic restrictive view
		else if (view.isSetGenericView()) {
			result.setViewName(Constants.VIEW_GENERIC_NAME);
			
			GenericViewType genericView = view.getGenericView();
			
			if (genericView.isSetParameterDefinitions() && genericView.getParameterDefinitions().sizeOfParameterDefinitionArray() > 0) {
				ParameterDefinition[] params = genericView.getParameterDefinitions().getParameterDefinitionArray();
				
				List<String> affectingParams = new ArrayList<String>();
				for (ParameterDefinition param : params) {
					affectingParams.add(param.getParameterIdentifier());	
				}
				
				result.addParameter("AffectingParameters", affectingParams);
			}
			
			if (genericView.isSetInsertCriteria() && genericView.getInsertCriteria().sizeOfInsertCriterionArray() > 0) {
				InsertCriterion[] insCrits = genericView.getInsertCriteria().getInsertCriterionArray();

				List<FilterType> filters = new ArrayList<FilterType>();
				for (InsertCriterion insCrit : insCrits) {
					filters.add(insCrit.getFilter());
				}
				
				result.addParameter("InsertCriteria", filters);
			}
			
			if (genericView.isSetReleaseCriteria() && genericView.getReleaseCriteria().sizeOfReleaseCriterionArray() > 0) {
				ReleaseCriterion[] relCrits = genericView.getReleaseCriteria().getReleaseCriterionArray();
				
				List<FilterType> releaseFilters = new ArrayList<FilterType>();
				for (ReleaseCriterion relCrit : relCrits) {
					releaseFilters.add(relCrit.getReleaseCondition().getFilter());
				}
				
				result.addParameter("ReleaseCriteria", releaseFilters);
			}
