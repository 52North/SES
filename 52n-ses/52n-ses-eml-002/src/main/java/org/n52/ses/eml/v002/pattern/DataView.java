		...
		if (this.esperString.equals("")) {
			 ...
			 */
				...
				if (this.parameters.containsKey("SelectFunctions")) {
					...
					List<SelFunction> funcs = (List<SelFunction>) this.parameters.get("SelectFunctions");
					for (SelFunction selFunction : funcs) {
						if (selFunction.getFunctionName().equals(DynamicSpatialBufferView.SELECTFUNCTION_NAME)) {
							/*
							 * #####
							 * this is the dynamic spatial buffer!
							 * #####
							 */
							...
							/*
							 * TODO the REAL stuff. skeleton is ready for it.
							 */
							Object tmp = this.parameters.get("InsertCriteria");
							
							FilterType buffer = null;
							if (tmp != null) {
								@SuppressWarnings("unchecked")
								List<FilterType> bufferList = (List<FilterType>) tmp;
								if (bufferList.size() > 0) buffer = bufferList.get(0);
							}

							if (buffer != null) {
								SpatialOpsType sot = buffer.getSpatialOps();

								QName sotQName = sot.newCursor().getName();

								if (SpatialFilterFactory.DWITHIN_QNAME.equals(sotQName)) {
									DWithinFilter filter = new DWithinFilter((DistanceBufferType) sot);
									filter.createExpressionString(false);

									this.esperString += "fromWKT(\""+ filter.getOriginalGeometry().toText() +"\"), "+ 
									filter.getDistance()+", \""+
									filter.getUom() +"\", \""+
									filter.getCRS() +"\", ";
								}
								...
								if (this.parameters.containsKey("AffectingParameters")) {
									@SuppressWarnings("unchecked")
									List<String> affecting = (List<String>) this.parameters.get("AffectingParameters");
									
									if (affecting.size() == 2) {
										this.esperString += affecting.get(0).replaceAll("/", ".")+ ", "+
											affecting.get(1).replaceAll("/", ".");
									}
								}
