...
public class PatternPlusNotMatchingXPathIT extends OvershootUndershootSubscriptionIT {
	
	private static final Logger logger = LoggerFactory.getLogger(PatternPlusNotMatchingXPathIT.class);
	
	@Override
	protected Subscription subscribe() throws OXFException,
			ExceptionReport, XmlException, IOException {
		String xml = readXmlContent("PatternPlusNotMatchingXPathSubscription.xml").replace("${consumer}", getConsumerUrl());
	...
	protected void evaluate(Object hasReceived) {
		Assert.assertNotNull("Noticiation was received but not expected!", hasReceived);
	}
